<?php
session_start();                        // abre la seccion
require_once("conexion.php");           //para se conecte con el php
...
echo"<html> 
       ...
       <body>
         <div class='banner'>SISTEMA DE SINDICATO DE TAXIS</div>";

if ((isset($_POST["accion"])) and ($_POST["accion"]=="Ingresar")){
    $nick = $_POST["nick"];                          //son variables para revisar datos
    ...
    $sql1 = $db->Prepare("SELECT clave
                          ...
                          AND estado <> 'X'   
                        ");
    $rs1 = $db->GetAll($sql1, array($nick)); ///ejecutar la consulta
    ...
    $sql2 = $db->Prepare("SELECT p.*
                          ...
                          WHERE u.usuario1 = ?
                          AND u.id_persona_1 = p.id_persona_1
                          AND p.estado <> 'X'   
                          AND u.estado <> 'X'   
                        ");
