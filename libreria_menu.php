...
if (isset($_SESSION["sesion_id_rol"])){
        ...
        if($cuerp == false){
            echo"<img src='../Sindicato_Taxis/logos/{$logo}'width='10%'>";

        }
        else{
            echo"<img src='privada/Sindicato_Taxis/logos/{$logo}'width='10%'>";
        }
	...
	$sql = $db->Prepare("SELECT ac.*, op.id_opcion, op.orden, op.cotenido, gr.id_grupo, gr.grupo, op.opcion 
					 FROM accesos ac
    ...
    $sql2 = $db->Prepare("SELECT ac.*, op.id_opcion, op.orden, op.cotenido, gr.id_grupo, gr.grupo, op.opcion 
					 FROM accesos ac
					 INNER JOIN opciones op ON ac.id_opcion = op.id_opcion
					 INNER JOIN grupos gr ON op.id_grupo = gr.id_grupo
					 WHERE ac.id_rol = '".$_SESSION["sesion_id_rol"]."'
                    ...
                    foreach ($rs2 as $t => $fila2) {
                        if ($grup==$fila2["grupo"]) {
                            if ($cuerp == false or $cuerp == ""){
                                echo"<li><a onclick='location.href=\"../".$fila2["cotenido"]."\"' ' style='cursor:pointer;'>".$fila2["opcion"]."</a></li>";
                            }
                                        else
                                        echo"<li><a onclick='location.href=\"sis_Sindicato_Velasquez_segundo_2022/".$fila2["cotenido"]."\"'  style='cursor:pointer;'>".$fila2["opcion"]."</a></li>";
                        }
                    }
                    echo"</ul>";
                echo"</li>";
